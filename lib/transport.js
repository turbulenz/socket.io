...
Transport.prototype.end = function (reason) {
  if (!this.disconnected) {
    ...
    this.clearTimeouts();
    var that = this;
    function transportEndTimeoutFn()
    {
      // No new transport since we saw the end
      if (!that.discarded)
      {
        that.log.info('transport end');

        that.close();
        that.disconnected = true;

        if (local) {
          that.manager.onClientDisconnect(that.id, reason, true);
        } else {
          that.store.publish('disconnect:' + that.id, reason);
        }
      }
    }

    setTimeout(transportEndTimeoutFn, 5000);
  }
};
