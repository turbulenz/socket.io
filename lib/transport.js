...
Transport.prototype.parseCookie = function parseCookieFn(str)
{
    var obj = {},
        i,
        len,
...
Transport.prototype.error = function (reason, advice) {
  ...
  var handshake = this.manager.handshaken[this.id];
  //this.log.warn(util.inspect(handshake));

  if (handshake)
  {
    var current = this.manager.transports[this.id];
    var headers = handshake.headers;
    var address = headers['x-forwarded-for'] ||
                  headers['X-Forwarded-For'] ||
                  handshake.address.address + ':' + handshake.address.port;

    var details = 'Client ' + current.name + ' transport error\n' +
                  'Address: ' + address + '\n' +
    ...
    if (headers.cookie)
    {
      var cookie = this.parseCookie(headers.cookie);
      if (cookie['gamesite'])
      {
        userId = cookie['gamesite'].slice(40);
  ...
  {
    details = 'Socket.IO error\n';
  }

  this.log.warn(details +
                'Reason: ' + reason + '\n' +
                'Advice: ' + (advice ? ('client should ' + advice) : ''));
  this.end('error');
};
